...
public class DryingRackBlockEntity extends BlockEntity {
    ...
    public ActionResult onUse(PlayerEntity player, Hand hand) {
        ...
        if (world != null && !world.isClient) {
            DryingRack.LOGGER.info("DryingRack onUse called - handStack: {}, isEmpty: {}",
                    handStack.getItem().toString(), handStack.isEmpty());
        }

        // Try to add item to empty slot
        if (!handStack.isEmpty()) {
            for (int slot = 0; slot < 4; slot++) {
                if (inventory.get(slot).isEmpty()) {
                     ...
                     * // Check if item has a drying recipe
                     ...
                     * 
                     * if (world != null && !world.isClient) {
                     * DryingRack.LOGGER.info("Slot {} is empty, recipe found: {}", slot, recipe !=
                     * null);
                     * }
                     * 
                     * if (recipe != null) {
                     ...
                     * 
                     * if (world != null && !world.isClient) {
                     * DryingRack.LOGGER.info("Item placed in slot {}", slot);
                     * }
                     * 
                     * return ActionResult.SUCCESS;
                     * }
            ...
            // Try to remove item from slot
            for (int slot = 3; slot >= 0; slot--) { // Remove from right to left
                if (!inventory.get(slot).isEmpty()) {
    ...
    private void sync() {
        if (world != null && !world.isClient) {
            ...
            // Force the client to re-render this block entity
